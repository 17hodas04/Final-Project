...
class FlappyDoom:
    ...
    def load_assets(self):
        # bird
        self.bird_images = [
            pg.image.load(f'assets/bird/{i}.png').convert_alpha()
            for i in range(3)
        ]

        bird_image = self.bird_images[0]
        bird_size = (
            int(bird_image.get_width() * BIRD_SCALE),
            int(bird_image.get_height() * BIRD_SCALE)
        )

        self.bird_images = [
            pg.transform.scale(img, bird_size)
            for img in self.bird_images
        ]
        ...
        # bird mask
        mask_image = pg.image.load('assets/images/mask.png').convert_alpha()
        mask_size = (
            int(mask_image.get_width() * BIRD_SCALE),
            int(mask_image.get_height() * BIRD_SCALE)
        )
